...
#		chomp(my $up   = $Repo->command("rev-parse", "rebase-merge/upstream"));
		...
		chomp(my $old  = $Repo->command("rev-parse", "rebase-merge/head"));
...
sub vis_xfrm
{
	...
	# C: commit
	# D:
	# e: merge visual left (â•”)
	# f: merge visual center (â•¦)
	# g: merge visual right (â•—)
	# I: straight line (â•‘)
	# K: branch visual split (â•¬)
	# m: single line (â”€)
	# O: overpass (â‰¡)
	# r: root (â•™)
	# t: tip (â•“)
	# x: branch visual left (â•š)
	# y: branch visual center (â•©)
	# z: branch visual right (â•)
	# *: filler
	...
	if ($Style == 1) {
		$s =~ tr/ABCD.efg.IKO.mrt.xyz/â”œâ”¤â”œâ”€.â”Œâ”¬â”.â”‚â”¼â‰¡.â”€â””â”Œ.â””â”´â”˜/;
	} elsif ($Style == 2) {
		$s =~ tr/ABCD.efg.IKO.mrt.xyz/â• â•£â•Ÿâ•.â•”â•¦â•—.â•‘â•¬â•.â”€â•™â•“.â•šâ•©â•/;
	} elsif ($Style == 10) {
		$s =~ tr/ABCD.efg.IKO.mrt.xyz/â”œâ”¤â”œâ”€.â•­â”¬â•®.â”‚â”¼â‰¡.â”€â””â”Œ.â•°â”´â•¯/;
	} elsif ($Style == 15) {
		$s =~ tr/ABCD.efg.IKO.mrt.xyz/â”£â”«â”£â”.â”â”³â”“.â”ƒâ•‹â˜°.â”â”—â”.â”—â”»â”›/;
	}
	return $s;
}
