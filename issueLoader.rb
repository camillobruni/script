...
`wget --tries=2 --timeout=3 --no-check-certificate "#{imageUrl}" --output-document="artifact#{issueNumber}.zip" \
...
`cd "#{destination}" && ( \
    mv "#{name}/#{name}.image" "Monkey#{issueNumber}.image"; \ 
    mv "#{name}/#{name}.changes" "Monkey#{issueNumber}.changes"; \ 
    mv "#{name}/PharoV10.sources" PharoV10.sources \ 
)`
